...
// This new version 2 July 2020 by Ian Wallace (IG: @ian_onahillroad) provides some statistical information adds an LCD switch, data logging and a laser saftey switch.
// This version 3 Oct 2022 revise Laser switch processin to improve handling - bounce code removed.  seems happier.
// this version 4 Oct 2022 uses SD card to remove memory limit on tests
// This program is described on my YouTube https://youtu.be/clALye887X4
...
const byte Version = 4;               // This application version
...
// ===============================================
//
String sDataFilename ;  // name of the file used as a memory extension to record results for stats
File Datafile;    // Memory extension file as no room for arrays without limit.
//
...
void setup() {      //This part of the program is run exactly once on boot
  ...
  if (!SD.begin(SDCARD_CS_PIN)) {
    ...
    // name and Just for good measure init the new data file
    sDataFilename = "Data_" + String(testrunnumber) + F(".csv");
    Datafile = SD.open(sDataFilename, FILE_WRITE);
    Datafile.close();
...
void DebugPrintExposureHistory() {
  // used for debug only - shows used contents of internal exposure history csv
  File Datafile = SD.open(sDataFilename, FILE_READ);
  String fileline;
  Serial.println("Index,Speed");
  while (Datafile.available()) {
    fileline = Datafile.readStringUntil('\n');
...

  {
    if (ShutterFired ) {
      ...
      if ((Speedinsecs <= LONGESTEXPOSURESECS) && (Speed > SHORTESTSENSIBLEEXP) ) {
        ...
        // log the exposure to SDcard (append)
        Datafile = SD.open(sDataFilename, FILE_WRITE);
        Datafile.print(bestmatchindex);
        Datafile.print(",");
        ...
        // sum - thus us a bit wastefull to do if we might not use it but I scrappped the array that kept counts to save memory to use sdcard
        // loop through the recorded stats
        long storedindex;
        long storedspeed;
        String fileline;
        Datafile = SD.open(sDataFilename, FILE_READ);
        while (Datafile.available()) {
          fileline=Datafile.readStringUntil('\n');
          storedindex = fileline.substring(0, fileline.indexOf(',')).toInt();
          storedspeed = fileline.substring(fileline.indexOf(',') + 1, fileline.length()).toInt();
          if (storedindex == bestmatchindex) {
            expsum = expsum + storedspeed;
            n++;
          }
        }
        Datafile.close();
        ...
        if (n >= NFORSHOWSTATS) {
          ...
          while (Datafile.available()) {
            ...
            if (storedindex == bestmatchindex) {
              ...
              sumsquarevar = sumsquarevar + sq(((float)storedspeed / 1000.0) - expmean);
